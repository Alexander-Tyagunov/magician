...
### Fixed
- **`bin/ctx` reported the wrong context window for every Claude 5 session.** `_effective_max()` assumed a 200K window and only inferred 1M once observed tokens *exceeded* 200K — but Opus 5, Sonnet 5, Fable 5, Opus 4.6/4.7/4.8, and Sonnet 4.6 are all natively 1M, so occupancy and the soft/firm/urgent band warnings were wrong for most of a session. It now resolves the window from the transcript's own model id (new `last_model()`, reusing the existing bounded tail scan — no extra I/O), honoring `CTX_MAX` first and `CLAUDE_CODE_DISABLE_1M_CONTEXT` second, and falling back to the previous observed-tokens heuristic when the model id is unknown or absent. Provider-prefixed ids (`anthropic.claude-sonnet-5`) resolve too; Haiku and the 4.5 generation stay at 200K. `ctx pct` now names the detected model.
- **`magician-ui automode` wrote a dead environment variable.** `CLAUDE_CODE_ENABLE_AUTO_MODE` was the Bedrock/Vertex/Foundry opt-in in Claude Code v2.1.158–2.1.206 and has had no effect since v2.1.207. Magician no longer writes it, and now *removes* one an earlier version left behind (on `automode`, `--available`, and `--off` alike). Unrelated `env` keys and your `statusLine` are untouched.

### Changed
- **`lore/models.md` rewritten around the Claude 5 family.** Current tiers with context, max output, pricing, and knowledge cutoffs (Opus 5's May 2026 cutoff is the freshest, which matters when a task turns on recent library behavior); the Claude Code alias table including `best`, `fable`, and `opusplan`, with the warning that alias resolution is **provider-dependent** (`opus` is still Opus 4.6 on Foundry); what no longer exists on current models (`budget_tokens`, sampling parameters, prefill, and the effort-gated `thinking: disabled`); and `ultracode` correctly described as a Claude Code setting rather than an API effort level.
- **Effort guidance is now capability intent, not a literal level.** `xhigh` does not exist on Opus 4.6 or Sonnet 4.6 — Claude Code silently clamps it to `high`, so every skill that said "raise `/effort xhigh`" was quietly giving worse advice on those models. All ~14 call sites (`/accelerate`, `/blueprint`, `/divine`, `/magic`, `/manifest`, `/orchestrate`, `/scrutinize`, `/sentinel`, `/transmute`, `/unravel`, `/ward`, `/weave`, and the divine references) now ask for **your model's deepest level** and name the fallback, backed by a per-model support matrix and named `deep`/`standard`/`cheap` intents in `lore/models.md`. Skills also stop suggesting a level at all on models with no effort axis.
- **Review lenses now optimize for recall, and say so.** Current models follow "be conservative" and "only report high-severity issues" literally — they investigate just as deeply, find the bugs, then decline to report them, which reads as a capability regression but is a prompt bug. `agents/{reviewer,sentinel,verifier,simplifier}.md` gain an explicit report-everything instruction and a `CONFIDENCE` field, and `/divine` and `/scrutinize` state the split plainly: finders cover, verification filters, and filtering can only drop what the finders surfaced.
- **`lore/autonomy.md` — auto mode brought current.** Auto mode needs no enabling on any provider and became the default permission mode for new Pro/Max/Team sessions on 2026-08-14, so magician now configures only the *starting* mode. Documents that `defaultMode: "auto"` is honored solely from `~/.claude/settings.json` (Claude Code ignores it from project/local settings so a repo can't grant itself auto mode), the real per-provider model requirements, `permissions.disableAutoMode` for admins, and that the classifier also reviews every `SendMessage`. `/statusline` guidance updated to match.
- **`/sentinel` and the `/divine` security lens know the scan can change models under them.** Opus 5 *and* Fable 5 run cybersecurity classifiers, and Claude Code answers a flag by re-running on a fallback model and **continuing the session there** (both fall back to Opus 4.8). A long sweep can finish on a different tier than it started on. Both skills now cover defensive framing, reporting the switch instead of presenting mixed-tier findings as one pass, and `switchModelsOnFlag` / `claude --safe-mode` for diagnosing a first-request flag that came from workspace context rather than the request.
- **Fan-out has a ceiling.** Opus 5 and Fable 5 delegate to subagents far more readily than earlier models, which multiplies cost and wall-clock on small work. `/orchestrate` and `/weave` now treat the parallelism map as a ceiling rather than a floor, prefer one capable agent over several, and never spawn an agent to re-check work another agent just did.
- **`/certify` and `/ward` distinguish tool evidence from self-review.** Every gate stays exactly where it was — the clean pass, the red-then-green test, the fresh-context adversarial verifier — but both now say why they are *not* the anti-pattern, so nobody layers "double-check your answer" on top of a command whose exit code is already the proof.

### Added
- **`lore/model-behavior.md`** — one place for how to prompt this generation, so skills stop repeating it. The anti-patterns that now actively cost quality (self-verification reminders, severity pre-filters, reasoning-echo instructions — which trip Fable 5's `reasoning_extraction` refusal — forced interim-status scaffolding, over-prescriptive skill bodies, and surfacing a token countdown *to the model*), and the behaviors that need explicit steering instead (length is prompted rather than dialed, delegation caps, scope boundaries, Sonnet 5's literalism, and grounding progress claims on long autonomous runs).
- **`lore/cross-session.md` + wiring into `/portal`, `/orchestrate`, and `/weave`** — Claude Code sessions can now message each other, which is exactly the gap in magician's parallel work: worktrees isolate the files, not the consequences. When a unit lands a change that breaks what a sibling session is building on, Claude hands it across with `SendMessage` instead of leaving the owner as the message bus. Documents the hard limits (plain text only, never history or files; a peer message can never approve anything; the auto-mode classifier reviews every send) and gates the whole thing behind feature detection — v2.1.224+, macOS/Linux, unavailable on Bedrock/Google Cloud/Foundry — so a missing feature is a silent no-op and no pipeline ever waits on a peer.

### Notes
- Sources: the Opus 5 / Sonnet 5 / Fable 5 model and prompting guides, the migration guide, the effort and task-budget references, and the Claude Code permission-modes, model-config, and cross-session-messaging docs, cross-checked against @ClaudeDevs through 2026-08-10. Model facts are point-in-time — `lore/models.md` says so and tells you to re-verify.
- Verified: all 29 Codex packaging/adapter/guard tests pass; the generated `plugins/magician/` package is in sync; `ctx` window resolution checked against a live Opus 5 transcript plus synthetic Haiku, Bedrock-prefixed, Sonnet 4.6, unknown-model, and no-model transcripts; `automode`/`--available`/`--off` exercised against an isolated settings file with the stale env var pre-seeded.
- Restart Claude Code to pick up the refreshed lore and skills.
