...
Read `../../references/codex-adapter.md`, then read `../../source-skills/jira/SKILL.md` and follow the source skill through that Codex adapter. Keep the source skill's gates and rules.
...
- **Config/secrets** — read `JIRA_BASE_URL` + a token (`JIRA_API_TOKEN`/`JIRA_PAT`/`JIRA_PROD_PAT`) from the environment. On missing config, use `../../source-skills/jira/setup.md` only as API guidance: explain how to provide variables to the current Codex environment or shell. Do not edit Claude settings, Codex global config, or secret files; never type/echo the secret.
