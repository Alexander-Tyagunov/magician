...
Codex equivalents:
- **HTTP** — use the bundled **`confluence` CLI** (on PATH when the plugin is enabled): `confluence whoami|get <id> [body]|search "<CQL>"|raw <METHOD> <path> [json]`. It wraps the REST API; don't hand-write `curl`, and **never use an ambient Confluence/Atlassian MCP** even if one appears in the tool list — use the CLI.
