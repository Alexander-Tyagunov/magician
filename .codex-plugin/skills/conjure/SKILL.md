...
**v3.8.0 — two-way design studio:** designs use the two-tier token system ([design-tokens.md](../../../skills/conjure/references/design-tokens.md)) so light/dark are ONE design and each run varies; GATE 3 asks target viewports. The browser streams clicks/selections + optional (opt-in) companion-chat to `state/events.jsonl` (read by cursor via the `events.json` endpoint); reply via `state/outbox.jsonl`. Use Browser Use for the pull path (read the live selection).
